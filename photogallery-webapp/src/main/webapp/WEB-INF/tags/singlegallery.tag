            ...
            <div class="simpleCart_shelfItem">
                ...
                <div class="input-append">
                    <select name="price" class="item_price"
                            <sec:authorize access="!hasRole('ROLE_USER')">disabled="true"</sec:authorize>>
                        <c:forEach items="${prices}" var="price">
                            <option format="${price.format}" format-id="${price.format.id}"
                                    paper-id="${price.paperType.id}" value="${price.priceImpl}">${price}</option>
                        </c:forEach>
                    </select>
                    <a class="btn item_add <sec:authorize access="!hasRole('ROLE_USER')">disabled</sec:authorize>"
                       href="javascript:"
                       title="
                       <sec:authorize access="!hasRole('ROLE_USER')"><spring:message code="page.public.you-should-be-logged-in"/></sec:authorize>
                       <sec:authorize access="hasRole('ROLE_USER')"><spring:message code="page.public.add-to-card"/></sec:authorize>"
                            >
                        <i class="icon-shopping-cart" title="
                       <sec:authorize access="!hasRole('ROLE_USER')"><spring:message code="page.public.you-should-be-logged-in"/></sec:authorize>
                       <sec:authorize access="hasRole('ROLE_USER')"><spring:message code="page.public.add-to-card"/></sec:authorize>"></i>
                    </a>
                </div>
            </div>
        </li>
